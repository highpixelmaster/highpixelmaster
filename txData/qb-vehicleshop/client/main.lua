...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn and QBCore ~= nil then
            local ped = PlayerPedId()
            local pos = GetEntityCoords(ped)
            if PlayerJob.name =="cardealer2" then
                ...
                for k, v in pairs(Config.Locations["stash"]) do
                    ...
                    if dist < 4.5 then
                            if dist < 1.5 then
                                sleep = 7
                                DrawText3Ds(v.x, v.y, v.z, "~g~E~w~ - Stash")
                                if IsControlJustReleased(0, 38) then
                                    TriggerEvent("inventory:client:SetCurrentStash", "cardealer2")
                                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "cardealer2", {
                                        maxweight = 100000,
                                        slots = 30,
                                    })
                                end
                            elseif dist < 2.5 then
                                DrawText3Ds(v.x, v.y, v.z, "Stash")
                            end  
                    end
                end
            end
