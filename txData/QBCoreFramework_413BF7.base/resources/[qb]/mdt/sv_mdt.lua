...
AddEventHandler("mdt:getOffensesAndOfficer", function()
	local usource = source
	local charges = {}
	exports.oxmysql:execute('SELECT * FROM fine_types', {
	}, function(fines)
		for j = 1, #fines do
...
AddEventHandler("mdt:performOffenderSearch", function(query)
	local usource = source
	local matches = {}
	exports.oxmysql:execute("SELECT * FROM `players` WHERE `charinfo` LIKE @query", {
...
AddEventHandler("mdt:getOffenderDetails", function(offender)
    ...
    exports.oxmysql:execute('SELECT * FROM `user_mdt` WHERE `char_id` = @id', {
		['@id'] = offender.id
	}, function(result)
        ...
        exports.oxmysql:execute('SELECT * FROM `user_convictions` WHERE `char_id` = @id', {
            ['@id'] = offender.id
            ...
            exports.oxmysql:execute('SELECT * FROM `mdt_warrants` WHERE `char_id` = @id', {
                ['@id'] = offender.id
            }, function(warrants)
...
AddEventHandler("mdt:saveOffenderChanges", function(id, changes, identifier)
	...
	exports.oxmysql:execute('SELECT * FROM `user_mdt` WHERE `char_id` = @id', {
		...
		if result[1] then
			exports.oxmysql:insert('UPDATE `user_mdt` SET `notes` = @notes, `mugshot_url` = @mugshot_url, `bail` = @bail WHERE `char_id` = @id', {
				['@id'] = id,
				['@notes'] = changes.notes,
			...
			exports.oxmysql:insert('INSERT INTO `user_mdt` (`char_id`, `notes`, `mugshot_url`, `bail`) VALUES (@id, @notes, @mugshot_url, @bail)', {
				['@id'] = id,
				['@notes'] = changes.notes,
				['@mugshot_url'] = changes.mugshot_url,
				['@bail'] = changes.bail
		...
		for i = 1, #changes.licenses_removed do
			...
			exports.oxmysql:insert('DELETE FROM `user_licenses` WHERE `type` = @type AND `owner` = @identifier', {
				['@type'] = license.type,
		...
		if changes.convictions ~= nil then
			for conviction, amount in pairs(changes.convictions) do	
				exports.oxmysql:insert('UPDATE `user_convictions` SET `count` = @count WHERE `char_id` = @id AND `offense` = @offense', {
					['@id'] = id,
					['@count'] = amount,
					['@offense'] = conviction
				})
		...
		for i = 1, #changes.convictions_removed do
			exports.oxmysql:insert('DELETE FROM `user_convictions` WHERE `char_id` = @id AND `offense` = @offense', {
				['@id'] = id,
...
AddEventHandler("mdt:saveReportChanges", function(data)
	exports.oxmysql:insert('UPDATE `mdt_reports` SET `title` = @title, `incident` = @incident WHERE `id` = @id', {
		['@id'] = data.id,
		['@title'] = data.title,
...
AddEventHandler("mdt:deleteReport", function(id)
	exports.oxmysql:insert('DELETE FROM `mdt_reports` WHERE `id` = @id', {
		['@id']  = id
	})
...
AddEventHandler("mdt:submitNewReport", function(data)
	...
	exports.oxmysql:insert('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`) VALUES (@id, @title, @incident, @charges, @author, @name, @date)', {
		['@id']  = data.char_id,
		['@title'] = data.title,
		['@incident'] = data.incident,
		['@charges'] = charges,
	...
	for offense, count in pairs(data.charges) do
		exports.oxmysql:execute('SELECT * FROM `user_convictions` WHERE `offense` = @offense AND `char_id` = @id', {
			['@offense'] = offense,
			['@id'] = data.char_id
		}, function(result)
			if result[1] then
				exports.oxmysql:insert('UPDATE `user_convictions` SET `count` = @count WHERE `offense` = @offense AND `char_id` = @id', {
					['@id']  = data.char_id,
					['@offense'] = offense,
					['@count'] = count + 1
				})
			else
				exports.oxmysql:insert('INSERT INTO `user_convictions` (`char_id`, `offense`, `count`) VALUES (@id, @offense, @count)', {
					['@id']  = data.char_id,
					['@offense'] = offense,
					['@count'] = count
				})
...
AddEventHandler("mdt:performReportSearch", function(query)
	local usource = source
	local matches = {}
	exports.oxmysql:execute("SELECT * FROM `mdt_reports` WHERE `id` LIKE @query OR LOWER(`title`) LIKE @query OR LOWER(`name`) LIKE @query OR LOWER(`author`) LIKE @query or LOWER(`charges`) LIKE @query", {
		['@query'] = string.lower('%'..query..'%') -- % wildcard, needed to search for all alike results
	}, function(result)
...
AddEventHandler("mdt:performVehicleSearch", function(query)
	local usource = source
	local matches = {}
	exports.oxmysql:execute("SELECT * FROM `player_vehicles` WHERE LOWER(`plate`) LIKE @query", {
		['@query'] = string.lower('%'..query..'%') -- % wildcard, needed to search for all alike results
	}, function(result)
		for index, data in ipairs(result) do
    ...
    if xPlayer.PlayerData.job.name == 'police' then
    	exports.oxmysql:execute("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(reports)
    		...
    		exports.oxmysql:execute("SELECT * FROM (SELECT * FROM `mdt_warrants` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(warrants)
    			...
    			exports.oxmysql:execute("SELECT * FROM `player_vehicles` WHERE `plate` = @query", {
					['@query'] = query
				}, function(result)
					local officer = GetCharacterName(usource)
...
AddEventHandler("mdt:getVehicle", function(vehicle)
	local usource = source
    exports.oxmysql:execute("SELECT * FROM `players` WHERE `citizenid` = @query", {
        ...
        exports.oxmysql:execute('SELECT * FROM `vehicle_mdt` WHERE `plate` = @plate', {
            ...
            exports.oxmysql:execute('SELECT * FROM `mdt_warrants` WHERE `char_id` = @id', {
                ...
                exports.oxmysql:execute('SELECT `bail` FROM user_mdt WHERE `char_id` = @id', {
                    ['@id'] = vehicle.owner_id
                }, function(bail)
...
AddEventHandler("mdt:submitNewWarrant", function(data)
	...
	data.date = os.date('%m-%d-%Y %H:%M:%S', os.time())
	exports.oxmysql:insert('INSERT INTO `mdt_warrants` (`name`, `char_id`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (@name, @char_id, @report_id, @report_title, @charges, @date, @expire, @notes, @author)', {
		['@name']  = data.name,
...
AddEventHandler("mdt:deleteWarrant", function(id)
	local usource = source
	exports.oxmysql:insert('DELETE FROM `mdt_warrants` WHERE `id` = @id', {
		['@id']  = id
	}, function()
		TriggerClientEvent("mdt:completedWarrantAction", usource)
	end)
...
AddEventHandler("mdt:saveVehicleChanges", function(data)
	...
	exports.oxmysql:execute('SELECT * FROM `vehicle_mdt` WHERE `plate` = @plate', {
		...
		if result[1] then
			exports.oxmysql:insert('UPDATE `vehicle_mdt` SET `stolen` = @stolen, `notes` = @notes WHERE `plate` = @plate', {
				['@plate'] = data.plate,
				['@stolen'] = data.stolen,
				['@notes'] = data.notes
			})
		else
			exports.oxmysql:insert('INSERT INTO `vehicle_mdt` (`plate`, `stolen`, `notes`) VALUES (@plate, @stolen, @notes)', {
				['@plate'] = data.plate,
				['@stolen'] = data.stolen,
				['@notes'] = data.notes
			})
