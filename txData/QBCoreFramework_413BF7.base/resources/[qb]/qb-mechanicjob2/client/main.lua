            ...
            if PlayerJob.name == "mechanic2" then
                    ...
                    if StashDistance < 20 then
                        ...
                        if StashDistance < 1 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerEvent("inventory:client:SetCurrentStash", "mechanicstash2")
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", "mechanicstash2", {
                                    maxweight = 4000000,
                                    slots = 500,
                                })
                            end
                ...
                if OnDutyDistance < 20 then
                    ...
                    if OnDutyDistance < 1 then
                        if onDuty then
                            TriggerEvent('cd_drawtextui:ShowUI', 'show', "[e] Clock Off Duty")
                        else
                            TriggerEvent('cd_drawtextui:ShowUI', 'show', "[e] Clock On Duty")
                        end
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("QBCore:ToggleDuty")
                        end
    ...
    QBCore.Functions.TriggerCallback('qb-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            QBCore.Functions.Progressbar("repair_part", "Repairing " ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
                ...
                TriggerServerEvent('qb-inventory:server:SaveStashItems', "mechanicstash2", StashItems)
